...
- A fase v2.7 permanece `Unreleased`: limites combinados por documento/worker, integração completa do preflight e do `tickAsync()` em Worker/desktop, cancelamento observado por cliente MCP externo, inspeção visual do painel e QA nativo ainda não foram concluídos.
