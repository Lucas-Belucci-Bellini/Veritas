...
- Documentada a proposta `docs/TAURI_SIMULATION_BOUNDARY.md` para o futuro comando Tauri/Rust: payload escalar versionado, validação duplicada fail-closed, canal de progresso bounded, cancelamento por `requestId`, teardown e golden parity antes de UI desktop. O shell nativo ainda é mínimo; comando, canal, engine Rust, build Windows/macOS/Linux e `Veritas-Setup.exe` permanecem `NOT IMPLEMENTED`/`NOT VERIFIED`.
