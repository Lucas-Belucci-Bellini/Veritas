...
- A fase v2.7 permanece `Unreleased`: supervisor, baseline bounded e preview de produção web foram concluídos em cenários limitados, mas integração Tauri/Rust, continuidade de estado Worker para `Step`/`Run`, cancelamento observado por host externo, inspeção visual ampla, medição de heap/carga sustentada e QA nativo ainda não foram concluídos.
