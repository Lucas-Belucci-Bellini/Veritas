...
- A fase v2.7 permanece `Unreleased`: supervisor, baselines bounded/sustentada, preview de produção web e parity DFF escalar foram concluídos em cenários limitados, mas contrato de checkpoint/continuidade entre requests, integração Tauri/Rust, cancelamento observado por host externo, inspeção visual ampla, medição efetiva isolada de heap/CPU e QA nativo ainda não foram concluídos.
