...
- As regressões do Simulator e do `documentRuntime` cobrem quota agregada entre instâncias, rollback de operações e liberação idempotente de memória estimada; os limites do worker dedicado, medição real de heap e uso desktop continuam fora deste slice.
...
- Registrada em `docs/EXECUTION_WORKER_BOUNDARY.md` a fronteira proposta para Worker/Tauri: protocolo versionado, `requestId`, budgets declarativos, cancelamento, cleanup, paridade e classificação de evidências; a implementação de Worker e a simulação Tauri/Rust continuam não iniciadas.
...
- A fase v2.7 permanece `Unreleased`: limites combinados por worker, integração completa do preflight e do `tickAsync()` em Worker/desktop, cancelamento observado por cliente MCP externo, inspeção visual do painel e QA nativo ainda não foram concluídos.
