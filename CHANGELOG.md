...
- A parity foi ampliada novamente com `tests/fixtures/worker-sequential-jk.json`, cobrindo hold (J=K=0), set (J=1/K=0), reset (J=0/K=1) e toggle (J=K=1) em sequência escalar. Worker, Simulator TypeScript e engine Rust coincidem nos 10 snapshots compartilhados; o teste passou focado e o Rust passou com **12 testes**. SR, delay, feedback, resume, vetores e runtime desktop continuam fora da evidência.
- A parity temporal agora inclui `tests/fixtures/worker-sequential-sr.json`, com hold, set, reset e o caso S=R=1 explicitamente definido como hold determinístico. Worker, Simulator TypeScript e engine Rust coincidem nos 9 snapshots compartilhados; o teste focado passou e o Rust passou com **13 testes**. Delay, feedback, resume, vetores e runtime desktop continuam fora da evidência.
- A parity temporal foi ampliada com `tests/fixtures/worker-sequential-delay.json`, cobrindo a subida e a descida de um atraso escalar de três tiques e a propagação separada até `out`. Worker, Simulator TypeScript e engine Rust coincidem nos 9 snapshots compartilhados; o teste focado passou e o Rust passou com **14 testes**. Feedback, resume, vetores e runtime desktop continuam fora da evidência.
- A matriz temporal foi fechada no escopo escalar inicial com `tests/fixtures/worker-sequential-feedback.json`. O DFF realimenta seu Q̄ interno para D, e seis pulsos manuais alternam o estado; Worker, Simulator TypeScript e engine Rust coincidem nos 7 snapshots públicos da saída primária. O teste focado passou e o Rust passou com **15 testes**. Q̄ não é exposto pelo snapshot Worker v1, embora permaneça parte do estado interno; resume, vetores e runtime desktop continuam fora da evidência.
- Foi criado um harness IPC Tauri controlado com `MockRuntime`: o `invoke` do comando nativo retornou snapshots e `requestId`, um listener recebeu evento bounded `veritas://simulation-progress`, e payload com campo desconhecido foi rejeitado antes da execução. A dependência `tauri/test` ficou restrita a dev-dependencies. O gate passou com **17 testes Rust**, mas isso é evidência de IPC em runtime simulado, não `RUNTIME VERIFIED` de uma janela desktop real nem cancelamento concorrente por invoke.
- O primeiro smoke desktop funcional opt-in foi executado no binário Linux empacotado com `--native-smoke` sob Xvfb. Após habilitar a capability mínima `core:event:default` para a janela `main`, o frontend realizou invoke real de `simulate_circuit_native`, recebeu **2 eventos de progresso** e **3 snapshots**, confirmou `requestId`/protocolo e gravou marcador local; o marcador e o processo foram removidos ao final. A evidência é `SMOKE VERIFIED` limitada a invoke/progresso do caso DFF escalar no Linux; editor, runtime canônico, cancelamento por invoke concorrente, AppImage independente, Windows e macOS continuam fora do gate.
- O smoke Linux opt-in `--native-smoke-cancel` agora realiza um segundo `invoke` concorrente de `cancel_circuit_native` durante uma execução escalar bounded de 256 nós/1.000 tiques. O request retornou `cancelled`, o cancelamento retornou `ok`, foram observados **61 eventos de progresso** e **0 eventos tardios**, com marcador local e processo removidos. A pausa de 25 ms usada para tornar a corrida reproduzível existe somente neste modo explícito de smoke; o runtime normal não é alterado. Isso é `SMOKE VERIFIED` limitado ao canal nativo Linux, não integração da UI canônica nem prova de interrupção física instantânea.
- O smoke Linux opt-in `--native-smoke-error` enviou um netlist serializável, mas semanticamente inválido, e confirmou rejeição `invalid-request` sem progresso. Uma tentativa posterior de cancelamento retornou `invalid-request`, comprovando que o registry foi limpo; também não houve eventos tardios. O marcador e o processo foram removidos. Isso é `SMOKE VERIFIED` limitado a erro fail-closed e teardown no canal nativo Linux, não cobertura de timeout, fechamento de janela, UI canônica ou demais plataformas.
...
- A fase v2.7 permanece `Unreleased`: supervisor, baselines bounded/sustentada, preview de produção web, parity DFF escalar, contrato/parser de checkpoint isolado e os smokes Linux opt-in de invoke/progresso, cancelamento concorrente e erro fail-closed/teardown foram concluídos em cenários limitados, mas integração de resume entre requests, uso Tauri/Rust pela UI canônica, timeout/fechamento de janela, inspeção visual ampla, medição efetiva isolada de heap/CPU e QA nativo multiplataforma ainda não foram concluídos.
- Iniciado o gate v2.8 de compatibilidade em `docs/V2_FORMAT_MIGRATION.md`, com inventário do Dexie v5, dos envelopes `veritas`/`veritas-circuits` v1 e das áreas ainda sem envelope de arquivo independente. `parseCircuitFile()` agora rejeita versões não inteiras, versões antigas sem migrador e coleções com projeto inválido sem filtragem silenciosa; 14 testes de persistência passaram. Isso é hardening de importação, não migração retroativa nem release.
...
- O envelope `veritas-testbenches` v1 passou a limitar cada lote a **256 documentos** e rejeitar nomes duplicados antes da escrita; serializer e parser agora mantêm esse limite simétrico. O teste focal passou com **9 casos**; a prova artificial de falha entre linhas continua pendente porque o caminho normal valida todo o lote antes da transação.
- A biblioteca `veritas-chip-library` v1 passou a limitar serializer e parser a **256 chips**, com timestamp único e incremento determinístico na importação para evitar empates de ordenação. A suíte focal passou com **8 casos**.
...
- Smoke visual real no preview de produção web local confirmou os controles atuais após limpeza de um service worker/cache antigo somente no origin `127.0.0.1:4173`: biblioteca vazia recusou exportação sem download, o chip `(8 Bits) 8-bit Adder` foi persistido localmente e exportado para `veritas-chip-library-v1.json` (30.734 bytes), uma cópia local renomeada foi importada e um JSON inválido com campo desconhecido foi rejeitado sem alterar os dois chips existentes. O input foi limpo, o erro permaneceu visível, `externalResources=[]` foi observado no Resource Timing e o banner `Nuvem desativada` permaneceu presente. Classificação: **`SMOKE VERIFIED` limitada ao preview web local e à rota opt-in da `ChipLibrary`**; não cobre Tauri, desktop/distribuição, cloud, login, sincronização, múltiplos perfis/dispositivos, persistência pós-reload, acessibilidade manual ampla, DLS arbitrário ou runtime de simulador. A ocorrência do cache foi condição de teste, não validação de atualização PWA; a fase v2.8 e o changelog continuam `Unreleased`.
