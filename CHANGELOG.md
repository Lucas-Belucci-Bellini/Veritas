...
- A fase v2.7 permanece `Unreleased`: integração do preflight e do `tickAsync()` com supervisor cross-worker/UI/desktop, paridade em Worker real no navegador, cancelamento observado por host externo, inspeção visual do painel, medição de heap e QA nativo ainda não foram concluídos.
