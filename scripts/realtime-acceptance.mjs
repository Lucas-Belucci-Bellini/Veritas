...
async function main() {
  ...
  const report = [
    ...
    `Execution mode: ${REALTIME_RUNNER_ALLOW_REAL && REALTIME_REQUIRE_REAL ? 'REAL_REQUIRED' : 'SAFE'}`,
    `Runner guard: REALTIME_RUNNER_ALLOW_REAL=${REALTIME_RUNNER_ALLOW_REAL ? '1' : '0'}`,
    `Required mode: RT_REQUIRE_REAL=${REALTIME_REQUIRE_REAL ? '1' : '0'}`,
