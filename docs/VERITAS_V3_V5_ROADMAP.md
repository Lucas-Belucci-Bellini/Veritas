...
**Estado de implementação atual:** classificação estática, budgets locais, quota agregada por documento/operação, cancelamento cooperativo do runtime, adapter MCP assíncrono, cleanup, o protocolo Worker web v1, o cliente hospedeiro, a ponte documental e o `DocumentWorkerExecutor` opt-in possuem código e regressões automatizadas. `docs/EXECUTION_WORKER_BOUNDARY.md` registra a fronteira e os gates seguintes; um smoke limitado no Chromium confirmou uma fixture direta e cancelamento após progresso, mas o Worker ainda não é orquestrado pela UI, não existe supervisor cross-worker nem simulação Tauri/Rust. O fechamento da fase ainda depende de paridade documental sob Worker real, integração completa do preflight/cancelamento nas superfícies UI/Worker/desktop, inspeção visual, backpressure e carga medida, medição de heap real e evidência nativa multiplataforma. Portanto, v2.7.0 continua `Unreleased`.
...
| Métricas de simulação ausentes | Não permite prometer performance | Medição isolada e repetível |
