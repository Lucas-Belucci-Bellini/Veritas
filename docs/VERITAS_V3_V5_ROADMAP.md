...
**Estado de implementação atual:** classificação estática, budgets locais, quota agregada por documento/operação, cancelamento cooperativo do runtime, adapter MCP assíncrono, cleanup, o protocolo Worker web v1, o cliente hospedeiro, a ponte documental, o `DocumentWorkerExecutor` opt-in, o `Preview Worker` explícito do painel temporal e o `SimulationWorkerSupervisor` bounded do host possuem código e regressões automatizadas. Smokes reais no Chromium confirmaram fixture direta, ponte documental, preview acionado pela UI, múltiplos Workers, fila bounded, rejeição explícita do excesso e cancelamento ativo/enfileirado, todos em circuitos escalares pequenos, sem substituir o runtime ativo. Uma baseline fixa registrou latências de 128,5–382,3 ms para cinco requests aceitos em uma rajada de oito. Depois, seis rodadas sustentadas repetíveis com 48 requests totais registraram 36 aceitos, 12 rejeições bounded, throughput descritivo de 68,926 outcomes/s, latência média de 57,944 ms e p95 de 89,000 ms, sempre com `active <= 2`, `queued <= 4` e cleanup zerado. A regressão e o smoke de parity da fixture `dff-clock` também produziram snapshots idênticos nos tiques 0–4 entre o Worker e o `Simulator` canônico dentro de um único request. O ensaio também observou `performance.memory.usedJSHeapSize` da página entre 47.001.860 e 48.326.352 bytes, sem tratar isso como heap isolado de Workers. `docs/EXECUTION_WORKER_BOUNDARY.md` e `docs/TAURI_SIMULATION_BOUNDARY.md` registram a fronteira web e a proposta nativa; o contrato/parser de checkpoint existe isoladamente e o primeiro comando Rust escalar já compila e passa em 11 testes Linux; o registry agora usa guarda RAII, rejeita duplicidade, permite cancelamento idempotente enquanto ativo e limpa o request após retorno antecipado, enquanto `yieldEvery` realiza `thread::yield_now()` e uma regressão observa cancelamento disparado por outra thread. A parity temporal foi ampliada com as fixtures compartilhadas TFF, JK, SR, delay e feedback: Worker e `Simulator` TypeScript, além do engine Rust, coincidem em snapshots de toggle/hold, hold/set/reset/toggle, hold/set/reset com S=R=1 determinístico, latência de subida/descida de três tiques e alternância por loop DFF/Q̄; DFF, TFF, JK, SR, delay e feedback são cobertura escalar incremental, não suporte vetorial nem prova de runtime desktop. Um harness IPC Tauri `MockRuntime` agora executa invoke de sucesso, recebe evento bounded e rejeita campo desconhecido fail-closed; isso é teste controlado do canal, não runtime real. O binário Linux empacotado também possui modo `--native-smoke` opt-in, capability mínima `core:event:default` e marcador local: sob Xvfb, o frontend executou invoke real do DFF escalar, recebeu dois eventos e três snapshots, e encerrou sem processo residual. O modo `--native-smoke-cancel` executou `cancel_circuit_native` por invoke concorrente durante uma cadeia escalar bounded de 256 nós/1.000 tiques, terminou com `cancelled`, recebeu 61 eventos e nenhum tardio, e também limpou o processo. O modo `--native-smoke-error` rejeitou uma referência de nó inexistente com `invalid-request`, não emitiu progresso, confirmou `invalid-request` ao tentar cancelar depois do retorno e não observou eventos tardios. Isso é `SMOKE VERIFIED` limitado ao canal nativo Linux e não integração do runtime canônico. Ainda não há resume entre requests ou integração de UI; timeout, fechamento de janela, edição/salvamento do projeto, AppImage independente, Windows/macOS e evidência desktop funcional completa continuam não verificados. O primeiro build Tauri Linux x86-64 também produziu e inspecionou `deb`/`AppImage` com checksums; em seguida, o workflow manual cross-platform produziu artefatos Linux, `Veritas-Setup.exe` Windows e bundle macOS arm64, todos em runners nativos e sem release. Como follow-up, o `.deb` do run cross-platform foi instalado, iniciado sob Xvfb por 10 segundos e removido no sandbox, sem crash observado: isso é `SMOKE VERIFIED` limitado a instalação/startup/cleanup Linux. A execução do comando Tauri, emissão de progresso, cancelamento externo e erro semântico fail-closed/teardown agora possuem `SMOKE VERIFIED` limitado no Linux por rotas opt-in controladas; timeout, fechamento de janela, uso do editor, simulação nativa pelo fluxo canônico, resume entre requests, AppImage em distribuição independente, inspeção visual multiplataforma, assinatura/notarização e evidência proporcional por usuário final permanecem `NOT VERIFIED`. O fechamento da fase ainda depende desses gates, além da medição efetiva isolada de heap/CPU. Portanto, v2.7.0 continua `Unreleased`.
...
O incremento atual desta fase está documentado em `docs/V2_FORMAT_MIGRATION.md`. O inventário confirma Dexie v5 e os envelopes `veritas`, `veritas-circuits` e `veritas-testbenches` v1. Os parsers rejeitam versões fora do contrato, campos desconhecidos, arquivos acima de 5.000.000 bytes e registros inválidos sem filtragem silenciosa; expressões usam o parser canônico, lotes de projetos/circuitos/testbenches usam transação Dexie e o hook de testbench confere existência/nome do circuito de destino. Isso é hardening fail-closed, não uma migração retroativa. Chips customizados e algoritmos foram inventariados: `veritas-custom-chip`/`veritas-chip-library` v1 cobrem serialização, refs file-local, remapeamento, importação atômica e controles locais opt-in; `veritas-algorithm`/`veritas-algorithms` v1 cobrem schema fechado, validação canônica, serializer e importação atômica sem executar expressões. A UI de persistência de algoritmos, Tauri/cloud e migrações portáteis legadas permanecem gates separados. A `ChipLibrary` recebeu smoke visual local no preview: exportação vazia foi recusada, um chip válido foi exportado e reimportado, e um arquivo semanticamente inválido foi rejeitado sem alterar os dois chips válidos; nenhuma origem externa apareceu nos recursos carregados. Isso é `SMOKE VERIFIED` limitado ao preview web local, não validação de distribuição desktop. A fase continua `Unreleased`.
