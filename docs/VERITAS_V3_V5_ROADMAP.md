...
**Estado de implementação atual:** classificação estática, budgets locais, quota agregada por documento/operação, cancelamento cooperativo do runtime, adapter MCP assíncrono, cleanup e o protocolo Worker web v1 possuem código e regressões automatizadas. `docs/EXECUTION_WORKER_BOUNDARY.md` registra a fronteira e os gates seguintes; o Worker ainda não é orquestrado pela UI, não existe supervisor cross-worker nem simulação Tauri/Rust. O fechamento da fase ainda depende de paridade em Worker real no navegador, integração completa do preflight/cancelamento nas superfícies Worker/desktop, inspeção visual, medição de heap real e evidência nativa multiplataforma. Portanto, v2.7.0 continua `Unreleased`.
