...
**Estado de implementação atual:** classificação estática, budgets locais, quota agregada por documento/operação, cancelamento cooperativo do runtime, adapter MCP assíncrono, cleanup, o protocolo Worker web v1, o cliente hospedeiro, a ponte documental, o `DocumentWorkerExecutor` opt-in e o `Preview Worker` explícito do painel temporal possuem código e regressões automatizadas. Um smoke limitado no Chromium confirmou fixture direta, ponte documental e preview acionado pela UI, todos em circuitos escalares pequenos, sem substituir o runtime ativo. `docs/EXECUTION_WORKER_BOUNDARY.md` registra a fronteira e os gates seguintes; não existe supervisor cross-worker, continuidade de estado Worker para `Step`/`Run` canônicos nem simulação Tauri/Rust. O fechamento da fase ainda depende de paridade sequencial, backpressure e carga medida, preview de produção, medição de heap real, inspeção visual multiplataforma e evidência nativa. Portanto, v2.7.0 continua `Unreleased`.
