...
| Chips customizados | IndexedDB `customChipProjects`, definição derivada de circuito local | Banco Dexie v5; definição interna `veritas-custom-chip` v1; envelope experimental `veritas-chip-library` v1 | Parser/serializer, limite simétrico de 256 chips, importação atômica com refs file-local e controles opt-in locais da `ChipLibrary` estão implementados e testados; Tauri/cloud permanecem fora |
| Testbenches | IndexedDB `testbenchProjects`, associado a `circuitId` | Banco Dexie v5; envelope de arquivo `veritas-testbenches` v1 | Parser/serializer v1 rejeitam versão não inteira/antiga/futura, campos desconhecidos, nomes duplicados, mais de 256 documentos, documentos inválidos e arquivo acima de 5.000.000 bytes; importação é transacional e a rota de UI confere existência/nome do circuito de destino |
| Algoritmos | IndexedDB `algorithmProjects`, com documento de fluxograma | Banco Dexie v5; documento `veritas-algorithm` v1; envelope experimental `veritas-algorithms` v1 | Parser/serializer, schema fechado, limite de 5.000.000 bytes, máximo de 256 projetos, validação canônica e importação Dexie atômica estão implementados; a UI de persistência ainda não expõe o fluxo |
...
No caso de algoritmos, `veritas-algorithm` v1 descreve um fluxograma e possui validador semântico; o storage e o envelope experimental `veritas-algorithms` v1 agora bloqueiam erros na criação, atualização e importação, sem executar strings de expressão. A UI atual mostra um workspace de execução de exemplo, mas não oferece ainda uma biblioteca persistente com controles de arquivo. A classificação correta é `PASSED` para o contrato/API local experimental de algoritmos e `NOT VERIFIED` para UI de persistência, Tauri, cloud e bibliotecas compartilhadas.

| Formato | O que já existe | O que não deve ser alegado |
|---|---|---|
| `veritas-custom-chip` v1 / `veritas-chip-library` v1 | Definição local validada, envelope de coleção experimental, refs file-local, ordenação de dependências, importação Dexie atômica e controles locais opt-in | Não é um fluxo de cloud/Tauri verificado; não há importação genérica de scripts/HDL/URLs |
| `veritas-algorithm` v1 / `veritas-algorithms` v1 | Documento versionado, validador de nós/targets/variáveis, envelope fechado/bounded, serializer e importação atômica local | Não é um fluxo de UI de persistência, não há migrador legado e não há execução de código importado |
