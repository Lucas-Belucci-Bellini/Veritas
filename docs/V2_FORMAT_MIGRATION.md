...
| Chips customizados | IndexedDB `customChipProjects`, definição derivada de circuito local | Banco Dexie v5; definição interna `veritas-custom-chip` v1 | O documento de definição e o pipeline de importação DLS já têm contratos próprios, mas não há envelope de coleção local para exportação/importação; gate explícito permanece pendente |
...
| Algoritmos | IndexedDB `algorithmProjects`, com documento de fluxograma | Banco Dexie v5; documento `veritas-algorithm` v1 | O documento e o validador semântico existem, mas não há envelope de coleção/parser de arquivo; versões antigas e shape fechado ainda não têm gate de importação |
...
Chips customizados e algoritmos não serão tratados como arquivos genéricos apenas porque seus documentos internos já carregam uma string `format` e uma versão. No caso de chips, `veritas-custom-chip` v1 descreve uma definição local validada e o pipeline DLS converte uma fonte externa em documentos Veritas; isso não equivale a um formato de coleção exportável. Instâncias de `custom-chip` devem continuar resolvendo apenas definições locais validadas, sem caminhos, URLs, scripts ou execução arbitrária.

No caso de algoritmos, `veritas-algorithm` v1 descreve um fluxograma e possui validador semântico, mas o storage atual guarda o documento diretamente no store Dexie e ainda não oferece parser/serializer de coleção. Até existir um envelope próprio com limites, schema fechado, validação de expressões e política de referências, a classificação correta é `NOT VERIFIED` para exportação/importação de algoritmos e bibliotecas de chips.

| Formato | O que já existe | O que não deve ser alegado |
|---|---|---|
| `veritas-custom-chip` v1 | Definição serializável local, validação hierárquica, limites de nesting/ciclos e importação DLS controlada | Não é ainda um arquivo de biblioteca portátil; não há importação genérica de scripts/HDL/URLs |
...
| Falha durante importação | Manter o banco inalterado por operação transacional ou por staging descartável | Implementado para importação de projetos, circuitos e testbenches via transação Dexie; falhas de parsing ocorrem antes da escrita. Migrações entre versões e dependências de múltiplos stores ainda pendentes |

## Próximos incrementos v2.8

A disciplina estrutural inicial dos envelopes `veritas`, `veritas-circuits` e `veritas-testbenches` está implementada no escopo de chaves permitidas, tipos básicos, limite de 5.000.000 bytes e validação semântica de expressões, com rejeição de campos desconhecidos e sem filtragem silenciosa. As escritas dos lotes de projetos e circuitos usam transação Dexie; a importação de testbenches também usa transação, embora a fixture de falha entre linhas ainda seja um gate separado. O próximo gate deve definir migradores explícitos para versões antigas ou confirmar a rejeição documentada de cada versão legada. Depois, cada formato que ganhar exportação deverá receber uma fixture atual, uma fixture de JSON quebrado, uma fixture de versão futura, uma fixture de versão antiga e uma fixture de shape semântico inválido; o testbench já possui cobertura inicial dessas categorias e precisa apenas ampliar a prova transacional e a política de associação ao circuito. A migração de IndexedDB deve ser documentada separadamente da migração de arquivos, pois abrir uma versão de banco e importar um arquivo são operações com riscos e rollback diferentes.

A associação entre circuito, chip customizado e testbench deve usar referências declaradas e validar dependências antes da escrita. A importação de testbench pelo hook da UI passa o nome do circuito selecionado; o backend confere que o `circuitId` ainda existe e que o nome não mudou, tudo dentro da transação Dexie. Um arquivo de circuito não deve carregar uma definição de chip arbitrária por caminho, URL ou script; quando a dependência não estiver no envelope permitido, a importação deve parar e explicar o que falta. O checkpoint nativo continuará isolado até existir uma política de identidade, linhagem, budget e restore transacional.
