...
No caso de algoritmos, `veritas-algorithm` v1 descreve um fluxograma e possui validador semântico; o storage agora bloqueia criação/atualização quando há erros, mas ainda guarda o documento diretamente no store Dexie e não oferece parser/serializer de coleção. Para chips, o envelope experimental possui limites, schema fechado, refs declaradas, importação transacional e controles opt-in locais na `ChipLibrary`, mas ainda não há evidência específica de distribuição desktop ou sincronização. A classificação correta é `PASSED` para o contrato/API e o fluxo local experimental de chips e `NOT VERIFIED` para exportação/importação de algoritmos, Tauri, cloud e bibliotecas compartilhadas.
