...
**Versão declarada no `package.json`:** `0.9.0-rc.4`
...
| --- | --- | --- |
| Versionamento | `package.json` está em `0.9.0-rc.4`; `v0.9.0-rc.3` está publicada e a nova RC está em preparação; ainda não há release estável. | Executar os gates da RC-4 e manter a sequência de pré-releases antes do estável. |
...
| `v0.9.0-rc.4` | Release candidate MCP-011 HTTP local | Testadores convidados e mantenedores | stdio preservado, HTTP local protegido, 8 checks HTTP PASS, quality/release workflows verdes e endpoint público ainda bloqueado até OAuth/HTTPS. |
