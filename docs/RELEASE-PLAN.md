...
**Versão declarada no `package.json`:** `0.9.0-rc.18`
...
| --- | --- | --- |
| Versionamento | `package.json` está em `0.9.0-rc.18`; `v0.9.0-rc.17` está publicada e ainda não há release estável. | Manter as pré-releases até concluir os gates reais de beta e estabilidade. |
...
| `v0.9.0-rc.17` | Release candidate CHIP-006 — chips na simulação temporal | Testadores convidados e mantenedores | Instâncias custom-chip achatadas antes do netlist temporal, reusando a elaboração do HDL; IDs de topo preservados para watch e linha do tempo; correção do `Simulator`, que ignorava a ligação de pinos de fronteira interna e rodava o chip com entrada em zero. |
| `v0.9.0-rc.18` | Release candidate CHIP-007 — chips do DLS com estrutura | Testadores convidados e mantenedores | Importação estrutural da biblioteca do Digital Logic Sim: a netlist é transcrita em vez de destilada, cada sub-chip vira instância e a hierarquia do autor continua navegável; 775 dos 1121 chips entram, contra 388 pelo caminho da expressão, e 212 deles foram cruzados com as tabelas verdade já derivadas, sem divergência; correção do defeito silencioso em que os pinos de um chip trocavam de lugar entre a definição e a elaboração. |
