...
O Veritas ainda é uma prévia `v0.9.0-rc.15`. A promoção beta depende de evidências reais de RLS cross-user, Realtime entre contas, inspeção em navegadores/dispositivos, onboarding externo e rollback operacional. O checklist automatizado confirma a presença do tutorial e dos contratos locais; ele não substitui uma pessoa externa seguindo este guia.
