...
| Release candidate | `v0.9.0-rc.11` | Nenhum requisito beta. O workflow executa testes, typecheck, lint, builds, readiness WASM-001, benchmark comparativo e smoke; a paridade WASM-002 é a próxima fatia opt-in. |
...
```bash
RELEASE_VERSION=v0.9.0-rc.11 npm run release:guard
```
