...
O protocolo Worker v1 (`workerProtocol.ts` + `simulation.worker.ts`) aceita somente netlists escalares dentro dos limites oficiais, valida o payload antes de criar o runtime, limita snapshots/progresso, propaga `requestId`, cancela cooperativamente, encerra por deadline, classifica budgets e ignora respostas após `dispose()`. Ele reutiliza o `Simulator` e não implementa ainda supervisor cross-worker, paridade com Worker real no navegador ou ponte Tauri/Rust.
