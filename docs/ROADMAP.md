...
| **v0.6.0** | Fortalecimento da simulação sequencial | Base de clock, delay, DFF, TFF, JK, SR, reset, Step/Run, timeline e waveform baseada nos snapshots existe nesta branch; demos `jk-clock`, `sr-clock`, `register-4bit` e `counter-4bit` estão disponíveis no workspace. Os budgets de `settle()` e de tiques totais agora são finitos, inteiros e limitados; `diagnoseSettle()` é exposto pela ponte `documentRuntime` com configuração por documento; `diagnoseDocumentRuntimePreview()` executa em uma cópia isolada e devolve diagnóstico, snapshot e estado final. Integração visual dessa preview, classificação de ciclos no grafo, budgets de operação/memória e QA de produto permanecem trabalho planejado. JK/SR usam borda de subida, Q/Q̄ e entradas ordenadas J/K/CLK ou S/R/CLK; o registrador captura D0–D3 em quatro DFFs com clock compartilhado; o contador usa quatro TFFs síncronos com carry AND e dois tiques de acomodação por pulso | Casos fundamentais determinísticos, limites de ticks, ausência de loops de UI, comparação temporal e regressão em cada release |
