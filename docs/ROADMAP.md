...
| **v0.6.0** | Fortalecimento da simulação sequencial | Base de clock, delay, DFF, TFF, JK, SR, reset, Step/Run, timeline e waveform baseada nos snapshots existe nesta branch; demos `jk-clock` e `sr-clock` estão disponíveis no workspace. Hardening de feedback e QA de produto permanecem trabalho planejado. JK/SR usam borda de subida, Q/Q̄ e entradas ordenadas J/K/CLK ou S/R/CLK | Casos fundamentais determinísticos, limites de ticks, ausência de loops de UI, comparação temporal e regressão em cada release |
