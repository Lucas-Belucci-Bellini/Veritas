...
A próxima etapa é `ALGO-002`: converter `ExecutionState` e trace em Watch de variáveis e `BranchTrace`, com valor anterior, tipo, expressão, operandos, resultado e ramo escolhido. Os exercícios dos materiais enviados foram mapeados em `docs/ALGO-002-LOGIC-MAPPING.md`: conectivos, implicação, tautologias, contradições, contrapositiva, contraexemplos e regras de inferência. Depois entram `ALGO-003` (loops), `ALGO-004` (funções/call stack) e `ALGO-005` (arrays/templates). Geração de código, I/O de arquivos, recursão irrestrita e recursos gráficos só entram após o IR, a segurança e os limites de execução estarem validados.
