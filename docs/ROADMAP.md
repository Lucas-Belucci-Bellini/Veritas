...
Este é um incremento técnico publicado como `Unreleased`. A fase v2.7 ainda não está concluída: o Simulator/documentRuntime já possuem `tickAsync()` com yield, timeout bounded, cancelamento e rollback, mas limites combinados por documento/worker, integração de superfície em Worker/desktop, smoke visual e QA nativo permanecem pendentes. Nenhuma tag ou GitHub Release foi criada.
