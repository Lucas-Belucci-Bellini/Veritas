...
| Canal de progresso | Eventos bounded por `requestId`, filtragem host-side, teardown e cancelamento cooperativo por `yieldEvery` | `PASSED` em testes Rust/TypeScript, no harness IPC `MockRuntime` e no smoke Linux empacotado: sucesso recebeu dois eventos; cancelamento concorrente recebeu 61 eventos bounded, terminou com `cancelled` e registrou zero eventos tardios. UI canônica e outras plataformas `NOT VERIFIED` |
