...
O caminho web já passou por build explícito do Worker, smoke local em Chromium, paridade de uma fixture direta versus Worker real, cancelamento real via cliente Worker, um preview documental acionado pela UI, testes determinísticos do supervisor, um smoke real do supervisor com múltiplos Workers, um preview de produção em origem limpa e paridade temporal DFF entre Worker e Simulator. O próximo passo é definir um checkpoint versionado para continuidade entre requests, sem promover essa paridade interna de um único request a substituição de `Step`/`Run`; medição efetiva isolada de heap/CPU e validação nativa continuam separadas. A ponte Tauri só poderá ser iniciada com comando Rust equivalente, teste de contrato, build nos três alvos e evidência separada de `BUILD VERIFIED`, `ARTIFACT VERIFIED`, `RUNTIME VERIFIED` e `SMOKE VERIFIED`. CI verde continuará sendo apenas evidência do workflow executado.

## Estado atual e próximos passos

Neste slice, o repositório possui `workerProtocol.ts`, `simulation.worker.ts`, `workerFactory.ts`, `workerClient.ts`, `documentWorker.ts`, `documentWorkerExecutor.ts` e `workerSupervisor.ts`, além de testes de endpoint/factory fake, a regressão `workerSequentialParity.test.ts` e o `Preview Worker` opt-in no painel temporal. Smokes adicionais executaram o Worker, o cliente, a ponte documental e o supervisor reais no Chromium local, incluindo paridade do snapshot final, cancelamento após progresso, preflight `acyclic`, imutabilidade do documento, preview isolado da UI, cinco requests independentes, concorrência máxima 2, cancelamento ativo/enfileirado, reservas zeradas após cleanup, preview de produção em origem limpa com asset JavaScript e paridade DFF nos tiques 0–4. O supervisor tem cobertura determinística para concorrência, fila, backpressure, reservas declarativas, cancelamento e dispose, e as rajadas bounded possuem baselines locais de latência; ainda não há medição efetiva isolada de consumo entre Workers nem integração de resume baseada no contrato de checkpoint entre requests. Essa evidência é limitada a fixtures pequenas; `Step`/`Run` continuam diretos, não há canais Tauri nem simulação Tauri/Rust, e a memória real continua não medida por Worker sob carga. Execução desktop interativa e suporte oficial a 5k/25k chips permanecem `NOT VERIFIED` ou `NOT SUPPORTED`, conforme o contrato aplicável.
