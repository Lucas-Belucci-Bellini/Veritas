...
| Preview diagnóstica isolada | `diagnoseDocumentRuntimePreview()` reconstrói uma cópia, restaura estado/aplica entradas opcionalmente e retorna diagnóstico, snapshot e estado final sem mutar o runtime original | PASSED em testes de documentRuntime; integração visual da preview, classificação de ciclos no grafo e smoke desktop permanecem `NOT VERIFIED` |
