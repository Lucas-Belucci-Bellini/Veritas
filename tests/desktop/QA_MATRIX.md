...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits` v1 e política de versão/migração fail-closed | PASSED no contrato `veritas-circuits`: versão não inteira/antiga/futura e projeto inválido são rejeitados; 14 testes de persistência passaram. Migração retroativa, envelope fechado para expressão/chip/testbench/algoritmo e preservação transacional de importação permanecem `NOT VERIFIED` |
