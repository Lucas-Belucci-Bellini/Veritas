...
| Diagnóstico cross-layer | `diagnoseDocumentRuntime()` expõe o diagnóstico pela ponte `CircuitDocument → Simulator`; `maxSettleTicks`, `maxTotalTicks`, budgets de operação e `AbortSignal` são configuráveis por documento | PASSED em testes de documentRuntime; integração visual, Worker/MCP/desktop, budgets de memória e smoke desktop permanecem `NOT VERIFIED` |
...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit` aplica budgets headless e cleanup | PASSED em 53 testes focados do domínio e 724 testes da suíte (1 skipped); adapter MCP passou regressões de safety; limites combinados por documento/worker, medição de heap, timeout assíncrono, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
