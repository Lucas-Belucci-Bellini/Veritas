...
| Paridade sequencial Worker | Fixtures `dff-clock`, `tff-clock`, `jk-clock` e `sr-clock` escalares, com etapas de entrada/clock e watches D/T/J/K/S/R/CLK/Q/OUT, comparadas contra snapshots do `Simulator` canônico | PASSED em 4 regressões determinísticas; `SMOKE VERIFIED` no Chromium com `SimulationWorkerClient` real, snapshots idênticos e `requestId` preservado para DFF, TFF, JK e SR. Delay, feedback, continuidade entre requests, vector/custom-chip, Step/Run canônicos e desktop permanecem `NOT VERIFIED` |
