...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits` v1 e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits` e `veritas`: versões não inteiras/antigas/futuras, campos desconhecidos, arquivos acima de 5.000.000 bytes, projetos inválidos e expressões não analisáveis são rejeitados sem filtragem silenciosa; importações de projetos e circuitos têm rollback transacional em falha de chave duplicada; 17 testes de circuitos e 16 de expressões passaram, 33 no total. Migração retroativa, envelope fechado para chip/testbench/algoritmo e dependências entre múltiplos stores permanecem `NOT VERIFIED` |
