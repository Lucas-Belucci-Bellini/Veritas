...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `tickAsync()` com yield/timeout, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit` aplica budgets headless e cleanup | PASSED em 57 testes focados do domínio e 728 testes da suíte (1 skipped); adapter MCP e regressões assíncronas passaram; painel expõe o status de segurança, mas inspeção visual, limites combinados por documento/worker, medição de heap, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
