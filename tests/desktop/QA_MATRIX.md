...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `tickAsync()` com yield/timeout, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit`/`simulateCircuitAsync` aplicam budgets headless e cleanup; o handler MCP encaminha `RequestHandlerExtra.signal`; `SimulatorExecutionBudget` agrega quotas entre runtimes | PASSED no gate local com 737 testes aprovados e 1 skipped; os três arquivos focados de Simulator/documentRuntime/MCP passaram com 111 testes; o typecheck, lint e builds web/MCP passaram. Inspeção visual, cancelamento por cliente externo, limites combinados por worker, medição de heap, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
