...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits`/`veritas-testbenches`/`veritas-chip-library` v1, documentos internos `veritas-custom-chip`/`veritas-algorithm` e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits`, `veritas`, `veritas-testbenches` e `veritas-chip-library`: versões não inteiras/antigas/futuras, campos desconhecidos, arquivos acima de 5.000.000 bytes, projetos/documentos inválidos e expressões não analisáveis são rejeitados sem filtragem silenciosa; importações de projetos, circuitos, testbenches e chips usam transação Dexie, a rota de UI de testbench confere existência/nome do circuito e a `ChipLibrary` oferece exportar/importar local opt-in; a biblioteca de chips remapeia refs file-local para ids novos e o store de algoritmos rejeita documento sem entrada; 17 testes de circuitos, 16 de expressões, 7 de testbench, 7 de chips e 5 de algoritmos passaram, 52 no total. Migração retroativa, dependências entre múltiplos stores, envelope portátil de algoritmos, validação visual/manual da UI de chips, Tauri/cloud e falha entre linhas de testbench permanecem `NOT VERIFIED` |
