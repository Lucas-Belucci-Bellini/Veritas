...
| Paridade sequencial Worker | Fixtures `dff-clock`, `tff-clock`, `jk-clock`, `sr-clock` e `delay` escalares, com etapas de entrada/clock, latência e watches D/T/J/K/S/R/CLK/DELAY/Q/OUT, comparadas contra snapshots do `Simulator` canônico | PASSED em 5 regressões determinísticas; `SMOKE VERIFIED` no Chromium com `SimulationWorkerClient` real, snapshots idênticos e `requestId` preservado para DFF, TFF, JK, SR e delay. Feedback, continuidade entre requests, vector/custom-chip, Step/Run canônicos e desktop permanecem `NOT VERIFIED` |
