...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total, rollback atômico, `cancel()`, `AbortSignal`, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções | PASSED em 44 testes focados e na suíte completa; classificação estática de ciclos, memória, timeout assíncrono, Worker/MCP/desktop e QA nativo permanecem `NOT VERIFIED` |
