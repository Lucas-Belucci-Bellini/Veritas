...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `tickAsync()` com yield/timeout, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit`/`simulateCircuitAsync` aplicam budgets headless e cleanup; o handler MCP encaminha `RequestHandlerExtra.signal` | PASSED em 57 testes focados do domínio; a suíte focada `mcp/src/tools.test.ts` passou com 56 testes, incluindo resultado assíncrono, abort prévio e abort entre yields; o typecheck valida o handler. A contagem total da suíte será registrada no gate final desta alteração. Inspeção visual, cancelamento por cliente externo, limites combinados por documento/worker, medição de heap, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
