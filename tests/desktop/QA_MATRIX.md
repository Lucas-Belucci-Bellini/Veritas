...
| Paridade sequencial Worker | Fixtures `dff-clock` e `tff-clock` escalares, com etapas de entrada/clock e watches D/T/CLK/Q/OUT, comparadas contra snapshots do `Simulator` canônico | PASSED em 2 regressões determinísticas; `SMOKE VERIFIED` no Chromium com `SimulationWorkerClient` real, snapshots idênticos e `requestId` preservado para DFF e TFF. JK, SR, delay, feedback, continuidade entre requests, vector/custom-chip, Step/Run canônicos e desktop permanecem `NOT VERIFIED` |
