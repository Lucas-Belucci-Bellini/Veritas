...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits` v1 e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits` e `veritas`: versões não inteiras/antigas/futuras, campos desconhecidos, arquivos acima de 5.000.000 bytes e projetos inválidos são rejeitados sem filtragem silenciosa; 16 testes de circuitos e 14 de expressões passaram, 30 no total. Migração retroativa, envelope fechado para chip/testbench/algoritmo e preservação transacional de importação permanecem `NOT VERIFIED` |
