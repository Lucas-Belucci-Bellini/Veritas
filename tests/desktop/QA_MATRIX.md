...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits` v1 e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits`, `veritas` e `veritas-testbenches`: versões não inteiras/antigas/futuras, campos desconhecidos, arquivos acima de 5.000.000 bytes, projetos/documentos inválidos e expressões não analisáveis são rejeitados sem filtragem silenciosa; importações de projetos e circuitos têm rollback transacional em falha de chave duplicada, testbenches usam transação Dexie e a rota de UI confere existência/nome do circuito de destino; 17 testes de circuitos, 16 de expressões e 7 de testbench passaram, 40 no total. Migração retroativa, dependências entre múltiplos stores, envelope de chips/algoritmos e falha entre linhas de testbench permanecem `NOT VERIFIED` |
