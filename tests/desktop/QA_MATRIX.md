...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Linux x86_64 | PASSED | PASSED | PASSED | SMOKE VERIFIED: `.deb` instalado e binário iniciado sob Xvfb; smoke opt-in `--native-smoke` também executou invoke/progresso real | Editor/Step/Run e fluxo interativo completo NOT VERIFIED | Instalação/remoção do `.deb`: SMOKE VERIFIED; marcador local do smoke removido; persistência: NOT VERIFIED | Prévia técnica |
