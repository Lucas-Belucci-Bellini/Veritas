...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `tickAsync()` com yield/timeout, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit`/`simulateCircuitAsync` aplicam budgets headless e cleanup; o handler MCP encaminha `RequestHandlerExtra.signal`; `SimulatorExecutionBudget` agrega quotas entre runtimes; `SimulationWorkerSupervisor` limita concorrência/fila e reservas declarativas no host | PASSED no gate local mais recente com 769 testes aprovados e 1 skipped em 87 arquivos aprovados e 1 skipped; os testes focados do supervisor/Worker passaram com 32 testes. Typecheck, lint, builds web/MCP/plugin passaram; o bundle plugin gerado foi restaurado antes do diff-check. Smoke web limitado do Worker/cliente/ponte/UI foi validado em Chromium; smokes reais do supervisor cobriram 5 requests com cancelamento e uma rajada de 8 requests com concorrência/fila bounded e rejeição explícita do excesso. Latências desse cenário foram `BASELINE RECORDED`; medição efetiva de operações/memória, carga sustentada, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
