...
| Execution Safety v2.7 — primeiro slice | `Simulator` com budgets de operações por tique/total e memória estimada, rollback atômico, `cancel()`, `AbortSignal`, `tickAsync()` com yield/timeout, `reset()` e `shutdown()` idempotente; `documentRuntime` encaminha as opções; preflight classifica SCCs; `simulateCircuit`/`simulateCircuitAsync` aplicam budgets headless e cleanup; o handler MCP encaminha `RequestHandlerExtra.signal`; `SimulatorExecutionBudget` agrega quotas entre runtimes | PASSED no gate local com 737 testes aprovados e 1 skipped; os três arquivos focados de Simulator/documentRuntime/MCP passaram com 112 testes; o typecheck, lint e builds web/MCP passaram. Inspeção visual, cancelamento por cliente externo, integração do supervisor cross-worker, paridade em Worker real, medição de heap, integração Worker/desktop e QA nativo permanecem `NOT VERIFIED` |
| Protocolo Worker isolado | `workerProtocol.ts` e `simulation.worker.ts` com protocolo v1, requestId, validação fail-closed, snapshots/progresso bounded, cancelamento, timeout, budget, deduplicação e dispose; factory de criação sob demanda | PASSED em 10 testes determinísticos com endpoint fake; `SMOKE VERIFIED` em Chromium local para uma fixture escalar pequena, com snapshots/progresso e paridade final. Backpressure sob carga, integração com UI/Tauri e runtime desktop permanecem `NOT VERIFIED` |
| Cliente hospedeiro Worker | `workerClient.ts` mantém um request ativo, entrega progresso, encaminha AbortSignal, envia cancel, classifica timeout como `forced-termination` e resolve dispose | PASSED em 6 testes com handle fake; `SMOKE VERIFIED` em Chromium local para resultado e cancelamento após progresso. Supervisor cross-worker, UI integrada, backpressure de navegador e runtime desktop permanecem `NOT VERIFIED` |
| Ponte CircuitDocument → Worker | `buildDocumentRuntimeNetlist()` e `buildDocumentWorkerRequest()` executam preflight, elaboração/canonicalização, validação de inputs/watches e rejeição do contrato vetorial v1 sem mutar o documento | PASSED em 6 testes documentais; a fixture do smoke real foi um Netlist escalar direto, portanto paridade documental no Worker real, UI, supervisor cross-worker e desktop permanecem `NOT VERIFIED` |
| Executor documental Worker | `DocumentWorkerExecutor` compõe bridge e cliente, devolve preflight/result, transforma falhas de construção em erro controlado e assume ownership até `dispose()` | PASSED em 4 testes com handle fake; execução documental em Worker real, UI, supervisor cross-worker e desktop permanecem `NOT VERIFIED` |
