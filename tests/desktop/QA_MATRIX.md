...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits` v1 e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits` e `veritas`: versões não inteiras/antigas/futuras e projetos inválidos são rejeitados sem filtragem silenciosa; 14 testes de circuitos e 12 de expressões passaram. Migração retroativa, envelope fechado para chip/testbench/algoritmo e preservação transacional de importação permanecem `NOT VERIFIED` |
