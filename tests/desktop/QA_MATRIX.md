...
| Paridade sequencial Worker | Fixture `dff-clock` do workspace, com quatro etapas de entrada/clock e watches D/CLK/Q/OUT, comparada contra snapshots do `Simulator` canônico | PASSED em 1 regressão determinística; `SMOKE VERIFIED` no Chromium com `SimulationWorkerClient` real, snapshots idênticos nos tiques 0–4 e `requestId` preservado. Continuidade entre requests, vector/custom-chip, Step/Run canônicos e desktop permanecem `NOT VERIFIED` |
...
| Primeiro comando Tauri/Rust escalar | DTOs Serde com `deny_unknown_fields`, validação bounded, budgets, snapshots finais e comandos `simulate_circuit_native`/`cancel_circuit_native` com `spawn_blocking` | PASSED em 4 testes Rust, `cargo test` e `cargo check` no Linux; `BUILD VERIFIED` para o crate. Golden DFF compartilhado é `PASSED` em TypeScript/Rust; canal de progresso, runtime interativo, Windows/macOS/Linux, artefatos nativos e `Veritas-Setup.exe` permanecem `NOT VERIFIED` |
