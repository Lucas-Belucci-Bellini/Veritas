...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Linux x86_64 | PASSED | PASSED | PASSED | SMOKE VERIFIED: `.deb` instalado e binário iniciado sob Xvfb; smoke opt-in `--native-smoke` executou invoke/progresso real, `--native-smoke-cancel` executou cancelamento concorrente e `--native-smoke-error` executou rejeição fail-closed/teardown | Editor/Step/Run e fluxo interativo completo NOT VERIFIED | Instalação/remoção do `.deb`: SMOKE VERIFIED; marcadores locais removidos; persistência: NOT VERIFIED | Prévia técnica |
...
| Paridade sequencial Worker | Fixtures `dff-clock`, `tff-clock`, `jk-clock`, `sr-clock`, `delay` e `feedback` escalares, com etapas de entrada/clock, latência, loop DFF/Q̄ e watches D/T/J/K/S/R/CLK/DELAY/Q/OUT, comparadas contra snapshots do `Simulator` canônico | PASSED em 6 regressões determinísticas; `SMOKE VERIFIED` no Chromium com `SimulationWorkerClient` real, snapshots idênticos e `requestId` preservado para DFF, TFF, JK, SR, delay e feedback. O snapshot público observa a saída primária; Q̄ permanece interno no loop. Continuidade entre requests, vector/custom-chip, Step/Run canônicos e desktop permanecem `NOT VERIFIED` |
...
| Formatos e migrações v2.8 | Inventário Dexie v5, envelopes `veritas`/`veritas-circuits`/`veritas-testbenches`/`veritas-chip-library`/`veritas-algorithms` v1, documentos internos `veritas-custom-chip`/`veritas-algorithm` e política de versão/migração fail-closed | PASSED nos parsers `veritas-circuits`, `veritas`, `veritas-testbenches`, `veritas-chip-library` e `veritas-algorithms`: versões não inteiras/antigas/futuras, campos desconhecidos, arquivos acima de 5.000.000 bytes, projetos/documentos inválidos e expressões não analisáveis são rejeitados sem filtragem silenciosa; importações de projetos, circuitos, testbenches, chips e algoritmos usam transação Dexie, a rota de UI de testbench confere existência/nome do circuito e a `ChipLibrary` oferece exportar/importar local opt-in; a biblioteca de chips remapeia refs file-local para ids novos e o store de algoritmos valida o grafo e não executa strings importadas; 17 testes de circuitos, 16 de expressões, 7 de testbench, 7 de chips e 8 de algoritmos passaram, 53 no total. Migração retroativa, dependências entre múltiplos stores, validação visual/manual da UI de chips, UI de persistência de algoritmos, Tauri/cloud e falha entre linhas de testbench permanecem `NOT VERIFIED` |
| Primeiro comando Tauri/Rust escalar | DTOs Serde com `deny_unknown_fields`, validação bounded, budgets, snapshots finais, cleanup RAII e comandos `simulate_circuit_native`/`cancel_circuit_native` com `spawn_blocking` | PASSED em 19 testes Rust, `cargo test` e `cargo check` no Linux; `BUILD VERIFIED` para o crate. Goldens DFF, TFF, JK, SR, delay e feedback compartilhados são `PASSED` em TypeScript/Rust; harness IPC `MockRuntime` executou `invoke` de sucesso, recebeu evento bounded com requestId e rejeitou payload desconhecido fail-closed. O binário empacotado Linux, com `--native-smoke` e capability `core:event:default`, executou invoke/progresso reais e gravou marcador local com 2 eventos/3 snapshots; o modo `--native-smoke-cancel` chamou cancelamento por invoke concorrente, terminou com `cancelled`, observou 61 eventos e zero tardios; `--native-smoke-error` rejeitou referência inexistente com `invalid-request`, confirmou ausência do request no registry e zero eventos/tardios. UI canônica, timeout/fechamento de janela, teardown de cancelamento e demais plataformas permanecem `NOT VERIFIED` |
| Bundles Tauri Linux alpha.1 | `Veritas_0.1.0-alpha.1_amd64.deb` e `Veritas_0.1.0-alpha.1_amd64.AppImage`, com inspeção de tipo, arquitetura, desktop entry, binário e SHA-256 | `BUILD VERIFIED`/`ARTIFACT VERIFIED` no Linux x86-64; `.deb` instalado, iniciado sob Xvfb e removido: `SMOKE VERIFIED` limitado; reconstrução oficial com capability de eventos, `--native-smoke` (invoke/progresso), `--native-smoke-cancel` (cancelamento concorrente, 61 eventos, zero tardios) e `--native-smoke-error` (erro fail-closed, cleanup e zero tardios) passou. AppImage em distribuição independente, editor/simulação funcional, assinatura, Windows/macOS e `Veritas-Setup.exe` permanecem `NOT VERIFIED` |
