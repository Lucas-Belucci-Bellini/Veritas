...
## Estado atual — Release 0.12.0 + shell desktop 0.1.0-alpha.1 + fundação da V1
...
| Shell desktop leve multiplataforma | `0.1.0-alpha.1` técnico: Tauri 2 sobre Vite/React, build Linux `.deb`/`.AppImage` validado; workflow nativo prepara Windows NSIS `.exe` e macOS `.app`/`.dmg` |
| Testes formais do aplicativo desktop | Reservados para `0.5.0`; instalação, offline, IndexedDB, simulação, importação/exportação, acessibilidade e remoção por plataforma |
| Promoção desktop `1.0.0` | Condicionada à estabilidade comprovada em Windows, macOS e Linux, sem bloqueios críticos, com atualização e assinatura verificadas |
...
| desktop 0.1.0-alpha.1 | Shell Tauri leve; Linux `.deb`/`.AppImage` validados e workflow para Windows `.exe` e macOS `.app`/`.dmg` |
