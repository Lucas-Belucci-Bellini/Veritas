...
O gate experimental `npm run beta:wasm` compila o núcleo para `wasm32-unknown-unknown`, verifica o ABI mínimo sem imports externos, instancia o módulo e mede tamanho, cold start e repetição. A prova opt-in `npm run beta:wasm:parity` executa a matriz VNET/VRES para netlists combinacionais uniformes de 1, 8, 32 e 64 bits, compara bytes, valores, saídas e ordem topológica com fixtures golden e verifica erros de fronteira; a especificação está em [`docs/WASM-NETLIST-ABI.md`](./docs/WASM-NETLIST-ABI.md). O `.wasm` permanece fora do bundle do navegador: a avaliação de circuitos e o fallback produtivo continuam em TypeScript.
